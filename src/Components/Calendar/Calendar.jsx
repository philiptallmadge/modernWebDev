...
const Calendar = () => {
        ...
        const fetchEventsAndRelatedData = async () => {
            try {
                ...
                if (isAuthenticated) {
                    ...
                    // If user is a venue, set their venue ID
                    if (user.get("userType") === "venue") {
                        const venuePointer = user.get("venue_pointer");
                        if (venuePointer) {
                            setVenueId(venuePointer.id);
                        }
                    }
                }
    ...
    const handleCreateEvent = async () => {
        ...
        try {
            const result = await createEvent(dateAndTime, venueId, null);
    ...
    const handleJoinEvent = async (eventId) => {
        ...
        if (userType !== "band") {
            alert("Only bands can join events.");
            return;
        }
        ...
        try {
            await updateEvent(eventId, bandPointer.id, null);
    ...
    const showCreateEventButton = userAuthenticated && currentUser?.get("userType") === "venue" && venueId;
                ...
                <ul className="event-list">
                    {events
                        ...
                        .map((event, index) => {
                            ...
                            const venueName = venue ? venue.get("Name") : "No venue yet";
                            ...
                            return (
                                <li key={index} className="event-card">
                                    ...
                                    {!band && userAuthenticated && currentUser?.get("userType") === "band" && (
                                        <button 
